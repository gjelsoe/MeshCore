...
void BaseChatMesh::onAdvertRecv(mesh::Packet *packet, const mesh::Identity &id, uint32_t timestamp,
  ...
  uint8_t saved_byte0 = app_data[0];
...
#ifdef USE_LZW
  if (parser.getFeat2() & ADV_CAP_LZW) {
    from->flags |= CONTACT_FLAG_SUPPORTS_LZW;
  } else {
    from->flags &= ~CONTACT_FLAG_SUPPORTS_LZW;
  }
#endif
...
#ifdef USE_LZW
    uint8_t flags = (data[4] & ~TXT_FLAG_COMPRESSED) >> 2;
#else
...
#ifdef USE_LZW
      char decomp_buf[MAX_TEXT_LEN + 1];
      const char *msg_text = (const char *)&data[5];
      uint32_t ack_hash;

      if (data[4] & TXT_FLAG_COMPRESSED) {
        // Read exact compressed length from 2-byte prefix
        int comp_len = ((int)data[5] << 8) | data[6];
        int dlen = mesh_text_decompress(&data[7], comp_len, decomp_buf, sizeof(decomp_buf));
        MESH_DEBUG_PRINTLN("decompress: compressed=%d, original=%d", comp_len, dlen);
        if (dlen > 0) msg_text = decomp_buf;
        // ACK hash uses exact length, not AES-padded len
        mesh::Utils::sha256((uint8_t *)&ack_hash, 4, data, 7 + comp_len, from.id.pub_key, PUB_KEY_SIZE);
      } else {
        // Uncompressed message — use strlen as normal
        mesh::Utils::sha256((uint8_t *)&ack_hash, 4, data, 5 + strlen((char *)&data[5]), from.id.pub_key,
                            PUB_KEY_SIZE);
      }

      onMessageRecv(from, packet, timestamp, msg_text);
#else
...
mesh::Packet *BaseChatMesh::composeMsgPacket(const ContactInfo &recipient, uint32_t timestamp,
  ...
  if (recipient.flags & CONTACT_FLAG_SUPPORTS_LZW) {
    ...
    MESH_DEBUG_PRINTLN("compress: original=%d, compressed=%d", text_len, comp_len);
    if (comp_len > 0) {
      temp[4] |= TXT_FLAG_COMPRESSED;
      // Store exact compressed length in 2-byte prefix
      temp[5] = (comp_len >> 8) & 0xFF;
      temp[6] = comp_len & 0xFF;
      memcpy(&temp[7], comp_buf, comp_len);
      text_len = 2 + comp_len;
    } else {
